  ...
  private:
    ...
    void publishData() {
        if (!position_queue_.empty()) {
            ...
            Eigen::Vector3f current_velocity =
                publishVelocity(filtered_position);
    ...
    Eigen::Vector3f publishVelocity(const Eigen::Vector4f& current_position) {
        ...
        if (time_difference > 0) {
            velocity[0] =
                (current_position[0] - previous_position_[0]) / time_difference;
            velocity[1] =
                (current_position[1] - previous_position_[1]) / time_difference;
            velocity[2] =
                (current_position[2] - previous_position_[2]) / time_difference;
    ...
    void publishAcceleration(const Eigen::Vector3f& current_velocity) {
        ...
        double time_difference =
            (current_time - previous_velocity_time_).seconds();
        Eigen::Vector3f acceleration = Eigen::Vector3f::Zero();

        if (time_difference > 0) {
            acceleration[0] =
                (current_velocity[0] - previous_velocity_[0]) / time_difference;
            acceleration[1] =
                (current_velocity[1] - previous_velocity_[1]) / time_difference;
            acceleration[2] =
                (current_velocity[2] - previous_velocity_[2]) / time_difference;
